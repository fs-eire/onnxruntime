...
import {createView, TensorView} from './tensor';
...
export class WebGpuBackend {
  ...
  getBuffer(gpuDataId: number): GPUBuffer {
    const buffer = this.gpuDataManager.get(gpuDataId);
    if (!buffer) {
      throw new Error(`no GPU data for buffer: ${gpuDataId}`);
    }
    return buffer.buffer;
  }
  createDownloader(gpuBuffer: GPUBuffer, size: number, type: 'float32'|'int32'): () => Promise<Tensor.DataType> {
    return async () => {
      const data = await downloadGpuData(this, gpuBuffer, size);
      return createView(data.buffer, type);
    };
  }
  createDisposer(gpuBuffer: GPUBuffer): () => void {
    return () => gpuBuffer.destroy();
  }
  // #endregion
}
